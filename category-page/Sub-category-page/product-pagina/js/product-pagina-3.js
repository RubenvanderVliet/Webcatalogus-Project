...
function fetchData() {
    return fetch('http://localhost:3000/nike-accesoires')
        .then(response => response.json());
}

// Function to create card elements dynamically for each accesoire
function createCard(accesoir) {
    // Generating HTML content for the card dynamically
    const cardHTML = `
        <div class="container">
            <div class="card">
                <div class="imgBx">
                    <img src="${accesoir.image}" alt="shoe-image">
                </div>
                <div class="contentBx">
                    <h2>${accesoir.name}</h2>
                    <div class="size">
                        <h3>Size :</h3>
                        ${accesoir.sizes.map(size => `<span>${size}</span>`).join('')}
                    </div>
                    <div class="color">
                        <h3>Color :</h3>
                        ${accesoir.colors.map(color => `<span style="background-color: ${color};"></span>`).join('')}
                    </div>
                    <a href="#">Buy Now</a>
                </div>
            </div>
...
function displayCards() {
    fetchData()
        .then(data => {
            ...
            // Looping through the accesoir data to create and display cards for each accesoir
            data.forEach(accesoir => {
                const card = createCard(accesoir); // Creating a card for the current accesoir
